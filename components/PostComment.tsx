...
import { Comment, Input, Row, Col, Button, message, Spin } from 'antd';
            ...
            <section className="make_comment">
                ...
                <Row justify="space-between" className="bottom">
                    <Col
                        ...
                        md={10}
                        lg={10}
                        xl={10}><Input addonBefore={<UserOutlined />} placeholder="输入你的昵称,默认值:匿名用户" onChange={(e) => setName(e.target.value)} allowClear /></Col>
                    <Col
                        xs={6}
                        sm={8}
                        md={10}
                        lg={10}
                        xl={10}><Button type="primary" size="middle" onClick={submitComment}>提交评论</Button></Col>
                </Row>
            </section>
                            ...
                            return (
                                <div key={item.Cid}>
                                    <Comment
                                        ...
                                        actions={[<span className="actions" onClick={clickHander(item.Cid)}>回复</span>]}
                                        datetime={
                                            ...
                                            item.children.map(item2 => {
                                                return (
                                                    <Comment
                                                        ...
                                                        datetime={
                                                            <span>{moment(item2.commentTime).format('YYYY-MM-DD HH:mm:ss')}</span>
                                                        }
                                                        key={item2.Cid}
                                                        className="nested"
                                                    />
                                            ...
                                            <section className="replay_comment">
                                                ...
                                                <Row justify="space-between" className="bottom">
                                                    <Col xs={16}
                                                        ...
                                                        md={10}
                                                        lg={10}
                                                        xl={10}><Input addonBefore={<UserOutlined />} placeholder="输入你的昵称,默认值:匿名用户" onChange={(e) => setReplyName(e.target.value)} allowClear /></Col>
                                                    <Col xs={7}
                                                        sm={8}
                                                        md={10}
                                                        lg={10}
                                                        xl={10} className="btns"><Button type="primary" size="small" onClick={submitReply}>回复</Button>
                                                        <Button type="primary" danger size="small" onClick={removeReplayInput}>取消</Button>
                                                    </Col>
                                                </Row>
                                            </section>
                                        )
